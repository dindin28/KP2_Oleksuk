...
Lumber::Lumber()
    : wood_(Wood()), sawing_option_(kBeam), delivery_date_(Date()), marking_(0), amount_(5), cost_(100)
{
}
...
Lumber::Lumber(Wood wood, SawingOption sawing_option, Date delivery_date,
    ...
    : wood_(wood), sawing_option_(sawing_option),
...
Lumber::Lumber(const Lumber &copy)
    : wood_(copy.wood_), sawing_option_(copy.sawing_option_),
...
//Getters
Wood Lumber::GetWood() const { return wood_; }
...
void Lumber::Print()
{
  std::cout << wood_ << ", " << SawingOptionToString(sawing_option_)
            << "(" << delivery_date_ << "), " << "marking: " << marking_ << ", amount: "
...
std::ostream &operator<<(std::ostream &out, const Lumber &copy)
{
  out << copy.GetWood() << ", " << SawingOptionToString(copy.GetSawingOption())
      << "(" << copy.GetDeliveryDate() << "), " << "marking: " << copy.GetMarking() << ", amount: "
